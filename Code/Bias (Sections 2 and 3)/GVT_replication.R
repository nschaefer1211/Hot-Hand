# This file replicates the work of GVT Table 4 using the raw data. It sets the basis for Tables 4, 8, and 9.
...
wb <- loadWorkbook("GilovichValloneTversky--CognitivePsychology--1985_CornellData.xls") # make sure to use the correct working directory
rawdata <- readWorksheet(wb, sheet = "Sheet1", header = TRUE)

####################### Creating data frame with all the necessary variables ###########################################################
...
# Creating the conditional probabilities
GVT_final <- GVT_table %>%
  mutate(
    `P(hit)` = FGM / shots, `P(hit|2 makes)` = FGM_2ma / shots_2ma, `P(hit|2 misses)` = FGM_2mi / shots_2mi,
...
# creating the data frame that can be used to create Tables 4, 8, and 9 (put number of shots in brackets etc.)
GVT_output <- GVT_final %>%
  select(
    shooter, `P(hit|4 misses)`, shots_4mi, `P(hit|3 misses)`, shots_3mi, `P(hit|2 misses)`, shots_2mi,
    `P(hit)`, shots, `P(hit|2 makes)`, shots_2ma, `P(hit|3 makes)`, shots_3ma, `P(hit|4 makes)`, shots_4ma
  ) %>%
  mutate(
    `GVT est. k = 2` = round(`P(hit|2 makes)` - `P(hit|2 misses)`, digits = 7),
    ...
    `P(hit|2 misses)` = paste(round(`P(hit|2 misses)`, digits = 3), paste("(", shots_2mi, ")", sep = "")),
    `P(hit)` = paste(round(`P(hit)`, digits = 7), paste("(", shots, ")", sep = "")),
    `P(hit|2 makes)` = paste(round(`P(hit|2 makes)`, digits = 3), paste("(", shots_2ma, ")", sep = "")),
  ...
  select(-shots_4mi, -shots_3mi, -shots, -shots_3ma, -shots_4ma, -shots_2mi, -shots_2ma)
